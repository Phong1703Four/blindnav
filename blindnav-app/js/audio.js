...
const AudioManager = {
  ...
  speak(text, lang = 'vi') {
    ...
    const chunks = this._splitText(text.trim(), 200);
  ...
  _playNextTts() {
    ...
    try {
      ...
      const url = `https://translate.google.com/translate_tts?ie=UTF-8&tl=${langCode}&client=tw-ob&q=${encodeURIComponent(item.text)}`;
      ...
      audio.onerror = () => {
        console.warn('Google TTS error, falling back to SpeechSynthesis');
        this._ttsPlaying = false;
        this._currentTtsAudio = null;
        // Fallback: dùng SpeechSynthesis
      ...
      audio.play().catch(() => {
        // Nếu autoplay bị chặn, dùng fallback
        this._ttsPlaying = false;
        this._currentTtsAudio = null;
        this._speakFallback(item.text, item.lang);
        setTimeout(() => this._playNextTts(), 100);
  ...
  _speakFallback(text, lang = 'vi') {
    ...
    const match = voices.find(v => v.lang.startsWith(lang === 'vi' ? 'vi' : 'en'));
  ...
  checkVietnameseSupport() {
    return true; // Luôn hỗ trợ vì dùng Google TTS
  },
